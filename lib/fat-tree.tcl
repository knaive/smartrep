...
Fat-tree instproc setup {ns_ n linkRate linkDelay hostDelay switchAlg enableSmartRep} {
    $self instvar ns
    ...
    for {set i 0} {$i < $host_num} {incr i} {
        ...
        $ns duplex-link $s($i) $tor($tor_index) [set link_rate]Mb [expr $link_delay+$host_delay] $queue_type
    ...
    for {set i 0} {$i < $agg_num} {incr i} {
        ...
        for {set j $first_tor} {$j <= $last_tor} {incr j} {
            $ns duplex-link $agg($i) $tor($j) [set link_rate]Mb [set link_delay] $queue_type
            #puts "agg($i) to tor($j)"
        }
    }
    ...
    for {set i 0} {$i < $pod_num} {incr i} {
        ...
        for {set j $first_agg} {$j <= $last_agg} {incr j} {
            for {set m 0} {$m < [expr $k/2]} {incr m} {
                $ns duplex-link $agg($j) $core($core_count) [set link_rate]Mb [set link_delay] $queue_type
...
Fat-tree instproc schedule {sim_end snode dnode flowsize interarrival} {
    ...
    $self instvar s host_num ct smartRep
    ...
    for {set i 0} {$i < $sim_end } {incr i} {
        ...
        if {![info exists cs_pairs($src,$dst)]} {
            ...
            $cs_pairs($src,$dst) setup $src $dst $smartRep
        }
