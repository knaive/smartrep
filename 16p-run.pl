...
foreach (@run) {
    ...
    foreach (@load) {
        ...
        foreach (@repflow_maxnum) {
            ...
            $pid = fork();
            if (!$pid) {
                simulate($cur_load,$repflow_num,$r);
                exit 0;
            } else {push @pids,$pid;}
        }
    }
...
sub simulate{
    my ($cur_load,$repflow_num,$run) = @_;
    my ($seed1,$seed2,$seed3) = get_seed($run);

    $output_dir = "/home/wfg/Desktop/stat-$sim_end/stat$run-$type-$sim_end";
    ...
    foreach (@routing_method) {
        ...
        my $arguments = "$sim_end $link_rate $link_delay $host_delay $queueSize $cur_load $spt $tor_num $spine_num $port_num $rm $repflow_num $miceflow_thresh $trace_file $seed1 $seed2 $seed3 $workload $meanFlowSize $topo $DCTCP";

        my $begin = qx(date +%s);
        print "Simulation $sim_end-$cur_load-$repflow_num-$rm started!\n";
        `ns smartRep.tcl $arguments > $log`;
